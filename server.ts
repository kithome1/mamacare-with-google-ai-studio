...
async function startServer() {
  ...
  // API status diagnostics endpoint to test and display cryptographic stats
  ...
  // API endpoint for symptom advice - SECURED WITH MIDDLEWARE
  app.post("/api/symptom-advice", requireAuth, async (req, res) => {
    try {
...
Write in a highly supportive, loving, maternal, and comforting tone.`;
      ...
      try {
        ...
        return res.json({
          advice: `[Local Fallback Mode] Due to connection latency or missing setup, we're serving prenatal guidelines:\n\n${fallback.advice}`,
          severity: fallback.severity,
          isFallback: true
        });
      } catch (err) {
        return res.status(500).json({ error: "Something went wrong providing advice." });
      }
    }
  });
